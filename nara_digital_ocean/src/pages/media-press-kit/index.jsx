...
const MediaPressKit = () => {
  ...
  const navTabs = [
    { id: 'home', label: t('media:tabs.home', 'Home'), icon: Icons.Home },
    { id: 'releases', label: t('media:tabs.releases', 'Press Releases'), icon: Icons.FileText },
    { id: 'assets', label: t('media:tabs.assets', 'Media Assets'), icon: Icons.Image },
    { id: 'stories', label: t('media:tabs.stories', 'Stories'), icon: Icons.BookOpen },
    { id: 'contacts', label: t('media:tabs.contacts', 'Media Contacts'), icon: Icons.Phone }
  ];

  if (loading && !dashboardData) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-slate-50 via-pink-50 to-blue-50 flex items-center justify-center">
        <div className="text-pink-900 text-xl">{t('common:loading', 'Loading...')}</div>
      </div>
    );
  }
        ...
        <div className="relative max-w-7xl mx-auto px-6 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <p className="text-xl text-pink-100 max-w-3xl mx-auto leading-relaxed" lang={currentLang}>
              {t('media:hero.description', 'Access press releases, high-resolution media assets, and story content for accurate reporting on NARA\'s marine research and conservation efforts')}
            </p>
            ...
            {dashboardData && (
              <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mt-12 max-w-4xl mx-auto">
                <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
                  ...
                  <div className="text-sm text-pink-200">{t('media:stats.pressReleases', 'Press Releases')}</div>
                </div>
                <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
                  <div className="text-3xl font-bold mb-2">{dashboardData.overview.totalAssets}</div>
                  <div className="text-sm text-pink-200">{t('media:stats.mediaAssets', 'Media Assets')}</div>
                </div>
                <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
                  <div className="text-3xl font-bold mb-2">{dashboardData.overview.totalStories}</div>
                  <div className="text-sm text-pink-200">{t('media:stats.stories', 'Stories')}</div>
                </div>
                <div className="bg-white/10 backdrop-blur-lg rounded-xl p-6 border border-white/20">
                  <div className="text-3xl font-bold mb-2">{dashboardData.overview.totalContacts}</div>
                  <div className="text-sm text-pink-200">{t('media:stats.mediaContacts', 'Media Contacts')}</div>
                </div>
              </div>
            )}
          </motion.div>
          ...
          <div className="flex flex-wrap items-center justify-between gap-4">
            <div className="flex items-center gap-6">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="text-gray-600">Last Updated: <strong>Today, 3:45 PM</strong></span>
              </div>
              <div className="flex items-center gap-2 text-sm">
                <Icons.Users className="w-4 h-4 text-pink-600" />
                <span className="text-gray-600"><strong>3</strong> Active Media Contacts</span>
              </div>
            </div>
            <div className="flex items-center gap-3">
              <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Share on Twitter">
                <Icons.Twitter className="w-5 h-5 text-gray-600" />
              </button>
              <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Share on Facebook">
                <Icons.Facebook className="w-5 h-5 text-gray-600" />
              </button>
              <button className="p-2 hover:bg-gray-100 rounded-lg transition-colors" title="Share on LinkedIn">
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="home" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              ...
                              <p className="text-xs text-gray-600 flex items-center gap-2">
                                ...
                                {release.publishDate?.toDate?.()?.toLocaleDateString()} • 2 min read
                              </p>
                            </div>
                          </div>
                      ...
                      <div className="p-6">
                        ...
                        <div className="flex items-center gap-3 pt-4 border-t border-gray-100">
                          <button className="flex-1 px-4 py-2 bg-pink-600 text-white font-medium rounded-lg hover:bg-pink-700 transition-colors flex items-center justify-center gap-2">
                            {t('common:readMore', 'Read Full Release')} <Icons.ExternalLink className="w-4 h-4" />
                          </button>
              ...
              <div className="mb-12">
                <h2 className="text-3xl font-bold text-gray-900 mb-6" lang={currentLang}>
                  {t('media:sections.featuredStories', 'Featured Stories')}
                </h2>
                    ...
                    <div key={story.id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all">
                      ...
                      <div className="p-6">
                        <div className="flex items-center gap-2 mb-3">
                          ...
                          <span className="px-2 py-1 bg-yellow-100 text-yellow-800 rounded-full text-xs font-bold">{t('media:featured', 'Featured')}</span>
                        </div>
                        ...
                        <button className="text-purple-600 hover:text-purple-700 font-medium text-sm">{t('media:readStory', 'Read Story')}</button>
                      </div>
                    </div>
                  ))}
                </div>
              ...
              <div>
                <h2 className="text-3xl font-bold text-gray-900 mb-6" lang={currentLang}>
                  {t('media:sections.quickDownloads', 'Quick Downloads')}
                </h2>
                <div className="grid md:grid-cols-4 gap-6">
                  {Object.entries(dashboardData.assetsByType).map(([type, count]) => (
                    ...
                    >
                      ...
                      <div className="font-bold text-gray-900 capitalize">{type}s</div>
                      <div className="text-sm text-gray-500 mt-1">{count} {t('media:files', 'files')}</div>
                    </button>
                  ))}
                </div>
              </div>
          ...
          {activeView === 'releases' && (
            <motion.div key="releases" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <h2 className="text-3xl font-bold text-gray-900 mb-6" lang={currentLang}>
                {t('media:sections.pressReleases', 'Press Releases')}
              </h2>
                      ...
                      <div className="flex-1">
                        ...
                        {release.attachments && release.attachments.length > 0 && (
                          <div className="flex items-center gap-2 text-sm text-gray-500 mb-4">
                            ...
                            <span>{release.attachments.length} {t('media:attachments', 'attachment(s)')}</span>
                          </div>
                        )}
                        <div className="flex items-center gap-4">
                          ...
                          <button className="text-pink-600 hover:text-pink-700 font-medium">{t('common:share', 'Share')}</button>
                        </div>
                      </div>
                    </div>
                  </div>
            ...
            <motion.div key="assets" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
                ...
                <div className="flex flex-wrap items-center justify-between gap-4">
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 mb-2" lang={currentLang}>
                      {t('media:sections.mediaAssets', 'Media Assets Library')}
                    </h2>
                    <p className="text-sm text-gray-600">High-resolution images, videos, logos, and documents for press use</p>
                  </div>
                  <div className="flex items-center gap-3">
                  ...
                  >
                    ...
                    <div className="relative h-56 bg-gradient-to-br from-pink-100 to-purple-100 overflow-hidden">
                      ...
                      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                        <button className="p-3 bg-white rounded-full hover:bg-pink-600 hover:text-white transition-colors" title="Preview">
                          <Icons.Eye className="w-5 h-5" />
                        </button>
                        <button className="p-3 bg-white rounded-full hover:bg-pink-600 hover:text-white transition-colors" title="Download">
                          <Icons.Download className="w-5 h-5" />
                        </button>
                        <button className="p-3 bg-white rounded-full hover:bg-pink-600 hover:text-white transition-colors" title="Share">
              ...
              {filteredAssets.length === 0 && (
                <div className="text-center py-20 text-gray-500 bg-white rounded-2xl shadow-lg border-2 border-dashed border-gray-300">
                  ...
                  <p className="text-lg font-semibold mb-2">{t('media:noItems', 'No assets found')}</p>
          ...
          {activeView === 'stories' && (
            <motion.div key="stories" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <h2 className="text-3xl font-bold text-gray-900 mb-6" lang={currentLang}>
                {t('media:sections.allStories', 'All Stories')}
              </h2>
                  ...
                  <div key={story.id} className="bg-white rounded-xl shadow-lg overflow-hidden hover:shadow-xl transition-all">
                    ...
                    <div className="p-6">
                      ...
                      <button className="w-full px-4 py-2 bg-purple-600 text-white rounded-lg hover:bg-purple-700 transition-colors font-medium">
                        {t('media:readStory', 'Read Full Story')}
                      </button>
                    </div>
                  </div>
                ))}
          ...
          {activeView === 'contacts' && dashboardData && (
            <motion.div key="contacts" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <h2 className="text-3xl font-bold text-gray-900 mb-6" lang={currentLang}>
                {t('media:sections.ourTeam', 'Our Media Team')}
              </h2>
              <div className="grid md:grid-cols-2 gap-6">
              ...
              {dashboardData.contacts.length === 0 && (
                <div className="text-center py-16 text-gray-500 bg-white rounded-xl">
                  ...
                  <p>{t('media:noItems', 'No items found')}</p>
                </div>
              )}
            </motion.div>
          )}
