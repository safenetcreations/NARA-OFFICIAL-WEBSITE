import React from 'react';
import { motion } from 'framer-motion';
import * as Icons from 'lucide-react';
import { useTranslation } from 'react-i18next';

const statConfig = {
  stations: { icon: Icons.Radio, color: 'blue' },
  uptime: { icon: Icons.Shield, color: 'green' },
  frequency: { icon: Icons.Clock, color: 'cyan' },
  charts: { icon: Icons.Map, color: 'indigo' },
  coverage: { icon: Icons.Activity, color: 'violet' },
  apis: { icon: Icons.Database, color: 'purple' }
};

const StatsOverviewSection = () => {
  const { t } = useTranslation('maritime');
  const stats = t('statsOverview.items', { returnObjects: true }) || [];

  return (
    <div className="py-12 bg-white border-y border-gray-200">
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-6">
          {stats.map((stat, index) => (
            <motion.div
              key={stat.id}
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
              viewport={{ once: true }}
              transition={{ delay: index * 0.1 }}
              className="text-center"
            >
              {(() => {
                const config = statConfig[stat?.id] || {};
                const IconComponent = config.icon || Icons.Info;
                const color = config.color || 'blue';
                return (
                  <>
                    <div className={`w-12 h-12 bg-${color}-100 rounded-lg flex items-center justify-center mx-auto mb-3`}>
                      <IconComponent className={`w-6 h-6 text-${color}-600`} />
                    </div>
                    <div className={`text-3xl font-bold text-${color}-600 mb-1`}>{stat?.value}</div>
                    <div className="text-sm font-semibold text-gray-900 mb-1">{stat?.label}</div>
                    <div className="text-xs text-gray-500">{stat?.sublabel}</div>
                  </>
                );
              })()}
            </motion.div>
          ))}
        </div>
      </div>
    </div>
  );
};

export default StatsOverviewSection;
